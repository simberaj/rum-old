...
class Attribute:
  ...
  TYPES = {'enumstring' : unicode, 'string' : unicode, 'float' : float, 'boolean' : bool, 'int' : int}
  UNIT_REMOVER = (lambda x: x.split()[0])
  TYPE_CONV = {unicode : None, float : UNIT_REMOVER, int : UNIT_REMOVER, bool : None}
  ...
  def get(self, values):
    try:
      ...
      elif self.typeConverter is not None and type(val) in (str, unicode):
        return self.type(self.typeConverter(val))
      else:
        return self.type(val)
    except ValueError, mess:
      return None
      # raise ValueError, '{} attribute get for {} failed: {}'.format(self.name, val, mess)
  ...
  def parseGetter(cls, config):
    if 'switch' in config:
      ...
      raise ConfigError, 'no getter found: ' + unicode(config)
...
def selector(config):
  if 'compound' in config:
    return COMPOUND_SELECTORS[config['compound']]([selector(clause) for clause in config['clauses']])
  else:
    attributeName = next(config.iterkeys())
    if isinstance(config[attributeName], list):
      ...
      raise ConfigError, 'cannot determine selection form: ' + unicode(config)
