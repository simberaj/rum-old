...
def median(numbers, sorted=False):
  if not sorted:
    numbers.sort()
...
def wsum(numbers, weights):
  return sum(numbers[i] * weights[i] for i in range(len(numbers)))

def wmean(numbers, weights):
  return wsum(numbers, weights) / sum(weights)

def wmedian(numbers, weights, sorted=False):
  fld = zip(numbers, weights)
  if not sorted:
    fld.sort(key=operator.itemgetter(0))
  whalf = sum(weights) / 2.0
  wcum = 0.0
...
WEIGHTED_FUNCTIONS = [wsum, wmean, wmedian]
